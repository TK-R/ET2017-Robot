...
/**
 * @brief imageData_t型構造体のメモリを確保し初期化する。
 *
 * @param[in] width   画像の幅
 * @param[in] height  画像の高さ
 * @param[in] type    色表現の種別
 * @return 初期化済みimageData_t型構造体
 */
imageData_t *allocate_image(uint32_t width, uint32_t height, uint8_t type) {
  uint32_t i;
  imageData_t *img;
  if ((img = calloc(1, sizeof(imageData_t))) == NULL) {
    return NULL;
  }
  img->width = width;
  img->height = height;
...
/**
 * @brief imageData_t型のクローンを作成する。
 *
 * カラーパレットやイメージデータは内部的に別にメモリを確保しているため、
 * allocateした後にmemcpyしてもクローンは作成できない。
 * この関数を使ってdeepcopyを行うこと。
 *
 * @param[in] img クローン元imageData_t型構造体
 * @return クローンされたimageData_t型構造体
 */
imageData_t *clone_image(imageData_t *img) {
  uint32_t i;
  imageData_t *new_img = allocate_image(img->width, img->height, img->color_type);
  if (new_img == NULL) {
    return NULL;
  }
...
/**
 * @brief imageData_t型構造体のメモリを開放する。
 *
 * 内部的に確保したメモリも開放する。
 * 内部メンバーのポインタを直接変更した場合
 * 正常に動作しないため注意。
 *
 * @param[in,out] img 開放するimageData_t型構造体
 */
 ...
 *
 * 指定引数のimageData_t型の内部表現を書き換えて戻すため、
 * 引数に指定したポインタと、成功時の戻り値は同じ値となる。
 * 元のimageData_t型は保持されないため、
 * 必要があれば予めクローンを作成しておく。
 *
 * @param[in,out] img 変換するimageData_t型へのポインタ
